...
export interface SpotifyTrailProps {
  /** Discord user ID tracked by Lanyard (join discord.gg/lanyard once). */
  discordId: string;
  /** Optional override — renders a fake "playing" track (visual only, no audio). */
  demoTrack?: {
    title: string;
    artist: string;
    albumArt: string | null;
  ...
  /** Drag threshold in pixels to trigger expand (default: 25) */
  dragThreshold?: number;
  /** Show cursor trail animation (default: true) */
  showTrail?: boolean;
}

const TrailParticle = ({ particle, index }: { particle: TrailParticle; index: number }) => {
  const opacity = 1 - index / 20;
  
  return (
    <Motion.div
      key={particle.id}
      className="pointer-events-none fixed size-1 rounded-full bg-[#1DB954]"
      initial={{ x: particle.x, y: particle.y, opacity: opacity * 0.6, scale: 1 }}
      animate={{ x: particle.x + particle.vx * 8, y: particle.y + particle.vy * 8, opacity: 0, scale: 0.5 }}
      transition={{ duration: 0.8, ease: "easeOut" }}
    />
  );
};

const SpotifyTrail = ({ discordId, demoTrack, dragThreshold = 25, showTrail = true }: SpotifyTrailProps) => {
  const { data, loading } = useSpotify({ discordId });
  ...
  // Generate trail particles during drag
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    // If dragged more than threshold, trigger expand
    ...
    if (showTrail && isDraggingRef.current) {
  ...
  const isOffline = !demoTrack && !data;
  const displayData = demoTrack
    ? { ...demoTrack, isPlaying: true }
    : data || {
        isPlaying: false,
        title: "Offline",
    ...
    <div className="relative z-40 my-6 h-14 w-full select-none">
        ...
        <div className="pointer-events-none fixed inset-0 z-40">
          <AnimatePresence>
            {particles.map((particle, index) => (
              <TrailParticle key={particle.id} particle={particle} index={index} />
            ))}
          </AnimatePresence>
        </div>
      )}
      ...
      <div className="absolute right-0 bottom-0 left-0 mx-auto flex w-fit items-end justify-center">
        <Motion.div
          ...
          className="group relative z-50 cursor-grab overflow-hidden border border-neutral-200/90 bg-white text-neutral-800 shadow-[0_1px_2px_rgba(0,0,0,0.04),0_8px_24px_-6px_rgba(0,0,0,0.1)] transition-cursor duration-150 dark:border-[#38332F] dark:bg-[#1A1715] dark:text-white dark:shadow-[0_1px_2px_rgba(0,0,0,0.2),0_8px_24px_-6px_rgba(0,0,0,0.4)] active:cursor-grabbing"
        >
          <Motion.div
            initial={false}
          ...
          <AnimatePresence mode="wait">
            {!isExpanded ? (
              ...
              <Motion.div
                ...
                style={{ top: EXPANDED_TEXT_TOP, height: `calc(100% - ${EXPANDED_TEXT_TOP}px)` }}
                className="absolute left-0 z-10 flex w-full flex-col items-center px-5 pb-8 pt-1 text-center"
              >
